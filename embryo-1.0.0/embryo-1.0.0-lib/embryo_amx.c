...
static int _embryo_program_init(Embryo_Program *ep, void *code)
{
	...
	if (hdr->magic != EMBRYO_MAGIC){
		return 0;
	}
	if ((hdr->file_version < MIN_FILE_VERSION)
			|| (hdr->ep_version > CUR_FILE_VERSION)){
		return 0;
	}
	...
	if ((hdr->flags & EMBRYO_FLAG_COMPACT)){
		return 0;
	}
...
EAPI Embryo_Program *embryo_program_new(void *data, int size)
{
	...
	if (size < (int) sizeof(Embryo_Header)){
		return NULL ;
	}
...
/// FIXME: are we interested in non freeable VMs?
#ifdef _NO_
/**
...
EAPI void embryo_program_free(Embryo_Program *ep) {
#ifdef _CHIBIOS_VM_
	// need to free code (Heap), Native Calls (Heap) and Program (Pool)
	...
	for (i = 0; i < ep->params_size; i++) {
		...
		if (ep->params[i].cell_array)
			free(ep->params[i].cell_array);
	}
	if (ep->params){
		free(ep->params);
	}
...
EAPI void embryo_program_native_call_add(Embryo_Program *ep, const char *name, Embryo_Cell (*func)(Embryo_Program *ep, Embryo_Cell *params))
{
	...
	for (i = 0; i < num; i++) {
		...
		func_entry = (Embryo_Func_Stub *) ((unsigned char *) func_entry
				+ hdr->defsize);
	}
}
...
EAPI int embryo_data_string_length_get(Embryo_Program *ep, Embryo_Cell *str_cell)
{
	...
	if ((!ep) || (!ep->base))
		return 0;
	hdr = (Embryo_Header *) ep->base;
	if ((!str_cell) || ((void *) str_cell >= (void *) (ep->base + hdr->stp))
...
EAPI void embryo_data_string_set(Embryo_Program *ep, const char *src, Embryo_Cell *str_cell)
{
	...
	for (i = 0; src[i] != 0; i++) {
		if ((void *) (&(str_cell[i])) >= (void *) (ep->base + hdr->stp))
			return;
		else if ((void *) (&(str_cell[i]))
				== (void *) (ep->base + hdr->stp - 1)) {
			str_cell[i] = 0;
			return;
		}
#ifdef WORDS_BIGENDIAN
...
EAPI Embryo_Status embryo_program_run(Embryo_Program *ep, Embryo_Function fn)
{
	...
	for (cycle_count = 0;;) {
		...
		*(Embryo_Cell *) (data + (int) alt) = pri;
		...
		BREAK;
		CASE(EMBRYO_OP_CALL);
		PUSH(((unsigned char *)cip - code) + sizeof(Embryo_Cell));
		/* skip address */
		...
		*(Embryo_Cell *) (data + (int) pri) -= 1;
		...
		CHKMEM(alt);
		CHKMEM(alt + offs);
		for (i = (int) alt; (size_t) offs >= sizeof(Embryo_Cell); i +=
				sizeof(Embryo_Cell), offs -= sizeof(Embryo_Cell))
		...
		ep->frm = frm;
		ep->stk = stk;
		num = _embryo_native_call(ep, pri, &pri,
				(Embryo_Cell *) (data + (int) stk));
		if (num != EMBRYO_ERROR_NONE) {
			if (num == EMBRYO_ERROR_SLEEP) {
				ep->pri = pri;
				ep->alt = alt;
		...
		ep->frm = frm;
		ep->stk = stk;
		num = _embryo_native_call(ep, offs, &pri,
				(Embryo_Cell *) (data + (int) stk));
		if (num != EMBRYO_ERROR_NONE) {
			if (num == EMBRYO_ERROR_SLEEP) {
				ep->pri = pri;
