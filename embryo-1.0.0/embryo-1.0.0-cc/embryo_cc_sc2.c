...
static void readline(char *line) {
	...
	do {
		if (!inpf || sc_eofsrc(inpf)) {
			...
			assert(skiplevel == 0);
			/* skiplevel was not stored on stack, because it should always be zero at this point */
			...
			if (lex(&val, &str) == tSYMBOL) {
				if (strcmp(str, "ctrlchar") == 0) {
					...
					if (*lptr == '"') {
						...
						for (i = 0;
								(i < (int) (sizeof(name))) && (alphanum(*lptr));
								i++, lptr++)
							name[i] = *lptr;
						name[i] = '\0';
					} /* if */
					...
					while ((*lptr <= ' ') && (*lptr != '\0'))
						lptr++;
					for (i = 0; (i < (int) (sizeof(name))) && (alphanum(*lptr));
							i++, lptr++)
						name[i] = *lptr;
					name[i] = '\0';
					...
					do {
						...
						while ((*lptr <= ' ') && (*lptr != '\0'))
							lptr++;
						for (i = 0;
								(i < (int) (sizeof(name))) && (isalpha(*lptr));
								i++, lptr++)
							name[i] = *lptr;
						name[i] = '\0';
						/* get the symbol */
...
static int substpattern(char *line, size_t buffersize, char *pattern,
		char *substitution) {
	int prefixlen;
	char *p, *s, *e, *args[10];
	int match, arg, len;
...
static void substallpatterns(char *line, int buffersize) {
	...
	while (*start != '\0') {
		...
		if (subst) {
			...
			if (!substpattern(start, buffersize - (start - line), subst->first,
					subst->second))
...
char *sc_tokens[] = { "*=", "/=", "%=", "+=", "-=", "<<=", ">>>=", ">>=", "&=",
		"^=", "|=", "||", "&&", "==", "!=", "<=", ">=", "<<", ">>>", ">>", "++",
		"--", "...", "..", "assert", "break", "case", "char", "const",
		"continue", "default", "defined", "do", "else", "enum", "exit", "for",
		"forward", "goto", "if", "native", "new", "operator", "public",
		"return", "sizeof", "sleep", "static", "stock", "switch", "tagof",
		"while", "#assert", "#define", "#else", "#emit", "#endif", "#endinput",
...
int lex(cell * lexvalue, char **lexsym) {
	...
	*lexsym = _lexstr;
	...
	if (newline) {
		...
		for (i = 0; i < (int) (lptr - pline); i++)
			if (pline[i] == '\t' && sc_tabsize > 0)
				stmtindent += (int) (sc_tabsize
						- (stmtindent + sc_tabsize) % sc_tabsize);
			else
				stmtindent++;
	} /* if */
	...
	if ((i = number(&_lexval, lptr)) != 0) { /* number */
	...
	} else if (*lptr == '\"' || (*lptr == sc_ctrlchar && *(lptr + 1) == '\"')) { /* unpacked string literal */
		_lextok = tSTRING;
		...
		*lexvalue = _lexval = litidx;
		...
		lptr = sc_packstr ?
				packedstring(lptr, rawstring) : unpackedstring(lptr, rawstring);
		if (*lptr == '\"')
			lptr += 1; /* skip final quote */
		else
			error(37); /* invalid (non-terminated) string */
	} else if ((*lptr == '!' && *(lptr + 1) == '\"')
			|| (*lptr == '!' && *(lptr + 1) == sc_ctrlchar
					&& *(lptr + 2) == '\"')
					|| (*lptr == sc_ctrlchar && *(lptr + 1) == '!'
							&& *(lptr + 2) == '\"')) { /* packed string literal */
		_lextok = tSTRING;
		...
		*lexvalue = _lexval = litidx;
		...
		if (rawstring)
			lptr += 1; /* skip "escape" character too */
		lptr = sc_packstr ?
				unpackedstring(lptr, rawstring) : packedstring(lptr, rawstring);
		if (*lptr == '\"')
			lptr += 1; /* skip final quote */
		else
			error(37); /* invalid (non-terminated) string */
...
int matchtoken(int token) {
	...
	if (tok == token || (token == tTERM && (tok == ';' || tok == tENDEXPR))) {
		return 1;
	} else if (!sc_needsemicolon && token == tTERM
	...
	while (root->next) {
		...
		if ((delete_labels || sym->ident != iLABEL)
				&& (delete_functions || sym->ident != iFUNCTN
						|| (sym->usage & uNATIVE) != 0)
						&& (delete_functions || sym->ident != iCONSTEXPR
								|| (sym->usage & uPREDEF) == 0)
								&& (delete_functions
										|| (sym->ident != iVARIABLE && sym->ident != iARRAY))) {
			root->next = sym->next;
			free_symbol(sym);
		} else {
			...
			if (sym->ident == iFUNCTN
					&& !isalpha(
							*sym->name) && *sym->name != '_' && *sym->name != PUBLIC_CHAR)sym ->usage&=
									~uPROTOTYPED;
...
find_symbol(symbol * root, char *name, int fnumber) {
	...
	while (ptr) {
		if (hash == ptr->hash && strcmp(name, ptr->name) == 0 && !ptr->parent
...
int refer_symbol(symbol * entry, symbol * bywhom) {
	...
	assert(bywhom != NULL);
	/* it makes no sense to add a "void" referrer */
	assert(entry != NULL);
	...
	for (count = 0; count < entry->numrefers && entry->refer[count]; count++)
		/* nothing */;
...
addsym(char *name, cell addr, int ident, int vclass, int tag, int usage) {
	...
	assert(
			!(ident == iFUNCTN || ident == iCONSTEXPR) || vclass != sGLOBAL || findglb(name) == NULL);
...
symbol* addvariable(char *name, cell addr, int ident, int vclass, int tag, int dim[], int numdim, int idxtag[])
{
	...
	assert(
			vclass != sGLOBAL || (sym = findglb(name)) == NULL || (sym->usage & uDEFINE) == 0);
